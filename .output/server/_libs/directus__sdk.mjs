...
const t$5 = [`fields`, `filter`, `search`, `sort`, `limit`, `offset`, `page`, `deep`, `backlink`, `alias`, `aggregate`, `groupBy`, `version`, `versionRaw`], n$3 = (e2) => typeof e2 == `boolean`, r$1 = (e2) => typeof e2 == `string` && !!e2, i = (e2) => typeof e2 == `number`, a = (e2) => Array.isArray(e2) && e2.length > 0, o = (e2) => typeof e2 == `object` && !!e2 && !a(e2) && Object.keys(e2).length > 0, s = (s2) => {
  let c = {};
  s2.fields && (a(s2.fields) && (c.fields = e$4(s2.fields).join(`,`)), r$1(s2.fields) && (c.fields = s2.fields)), o(s2.filter) && (c.filter = JSON.stringify(s2.filter)), r$1(s2.search) && (c.search = s2.search), s2.sort && (a(s2.sort) && (c.sort = s2.sort.join(`,`)), r$1(s2.sort) && (c.sort = s2.sort)), `limit` in s2 && (i(s2.limit) && s2.limit >= -1 && (c.limit = String(s2.limit)), r$1(s2.limit) && (c.limit = s2.limit)), `offset` in s2 && (i(s2.offset) && s2.offset >= 0 && (c.offset = String(s2.offset)), r$1(s2.offset) && (c.offset = s2.offset)), `page` in s2 && (i(s2.page) && s2.page >= 1 && (c.page = String(s2.page)), r$1(s2.page) && (c.page = s2.page)), o(s2.deep) && (c.deep = JSON.stringify(s2.deep)), o(s2.alias) && (c.alias = JSON.stringify(s2.alias)), o(s2.aggregate) && (c.aggregate = JSON.stringify(s2.aggregate)), s2.groupBy && (a(s2.groupBy) && (c.groupBy = s2.groupBy.join(`,`)), r$1(s2.groupBy) && (c.groupBy = s2.groupBy)), r$1(s2.version) && (c.version = s2.version), s2.versionRaw && (n$3(s2.versionRaw) && (c.versionRaw = String(s2.versionRaw)), r$1(s2.versionRaw) && (c.versionRaw = s2.versionRaw));
...
const n$1 = (n2, r2) => () => (e(String(n2), `Collection cannot be empty`), t(n2, `Cannot use readItems for core collections`), { path: `/items/${n2}`, params: r2 ?? {}, method: `GET` });
const n = {}, r = (r2 = {}) => (i2) => {
  let a2 = { ...n, ...r2 };
